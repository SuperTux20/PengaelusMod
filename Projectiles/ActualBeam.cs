...
namespace PengaelusMod.Projectiles {
	public class ActualBeam : ModProjectile {
		...
		public override bool PreAI() {
			...
			if (Projectile.velocity.HasNaNs() || Projectile.velocity == Vector2.Zero) Projectile.velocity = -Vector2.UnitY;
			...
			while ((float)num812 < num811) {
				...
				Tuple<int, int> tuple;
				float num818;
				if (!Collision.TupleHitLine(num814, num815, num816, num817, 0, 0, new List<Tuple<int, int>>(), out tuple)) num818 = new Vector2((float)Math.Abs(num814 - tuple.Item1), (float)Math.Abs(num815 - tuple.Item2)).Length() * 16f;
				else if (tuple.Item1 == num816 && tuple.Item2 == num817)num818 = 2400f;
				else num818 = new Vector2((float)Math.Abs(num814 - tuple.Item1), (float)Math.Abs(num815 - tuple.Item2)).Length() * 16f;
				array3[num812] = num818;
				num812++;
			}

			float num819 = 0f;
			for (int num820 = 0; num820 < array3.Length; num820++) num819 += array3[num820];
			num819 /= num811;
			float amount = 0.5f;
		...
		public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox) {
			float tmp = 0f;
			if (Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), Projectile.Center, Projectile.Center + Projectile.velocity * Projectile.localAI[1], 30f * Projectile.scale, ref tmp)) return true;
			return false;
		}
