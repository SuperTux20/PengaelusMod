...
namespace PengaelusMod.Projectiles {
	...
	public static class ProjectileExtras {
		...
		public static void HomingAI(ModProjectile modProj, NPC target, float velocity = 4f, float acceleration = 0.1f) {
			...
			if (acceleration * acceleration >= diff.LengthSquared()) projectile.velocity = aim;
			else {
		...
		public static NPC FindCheapestNPC(Vector2 position, Vector2 velocity, float acceleration, float maxAngle, float maxDist = 2000f, bool ignoreLineOfSight = false, bool ignoreFriendlies = true, bool ignoreDontTakeDamage = false, bool ignoreChaseable = false) {
			...
			if (maxAngle > Math.PI / 2) maxDeviation = -maxDeviation;
		...
		public static void Bounce(ModProjectile modProj, Vector2 oldVelocity, float bouncyness = 1f) {
			Projectile projectile = modProj.Projectile;
			if (projectile.velocity.X != oldVelocity.X) projectile.velocity.X = -oldVelocity.X * bouncyness;
			if (projectile.velocity.Y != oldVelocity.Y) projectile.velocity.Y = -oldVelocity.Y * bouncyness;
		}
		...
		public static void YoyoAI(int index, float seconds, float length, float acceleration = 14f, float rotationSpeed = 0.45f, ExtraAction action = null, ExtraAction initialize = null) {
			...
			for (int i = 0; i < projectile.whoAmI; i++) if (Main.projectile[i].active && Main.projectile[i].owner == projectile.owner && Main.projectile[i].type == projectile.type) flag = true;
			if (projectile.owner == Main.myPlayer) {
				projectile.localAI[0] += 1f;
				if (flag) projectile.localAI[0] += (float)Main.rand.Next(10, 31) * 0.1f;

				float num = projectile.localAI[0] / 60f;
				num /= (1f + Main.player[projectile.owner].GetAttackSpeed(DamageClass.Melee)) / 2f;
				if (num > seconds) projectile.ai[0] = -1f;
			}
			...
			if (Utils.HasNaNs(projectile.velocity)) projectile.Kill();

			projectile.timeLeft = 6;
			float num2 = length;
			if (Main.player[projectile.owner].yoyoString) num2 = num2 * 1.25f + 30f;
			...
			if (projectile.ai[0] >= 0f) {
				if (projectile.velocity.Length() > num3) projectile.velocity *= 0.98f;
				bool flag3 = false;
				bool flag4 = false;
				Vector2 vector = Main.player[projectile.owner].Center - projectile.Center;
				if (vector.Length() > num2) {
					flag3 = true;
					if (vector.Length() > num2 * 1.3f) flag4 = true;
				}
				...
				if (projectile.ai[0] >= 0f) {
					if (flag3) {
						num4 /= 2f;
						num3 *= 2f;
						if (projectile.Center.X > Main.player[projectile.owner].Center.X && projectile.velocity.X > 0f) projectile.velocity.X = projectile.velocity.X * 0.5f;
						else if (projectile.Center.X < Main.player[projectile.owner].Center.X && projectile.velocity.X > 0f) projectile.velocity.X = projectile.velocity.X * 0.5f;
						if (projectile.Center.Y > Main.player[projectile.owner].Center.Y && projectile.velocity.Y > 0f) projectile.velocity.Y = projectile.velocity.Y * 0.5f;
						else if (projectile.Center.Y < Main.player[projectile.owner].Center.Y && projectile.velocity.Y > 0f) projectile.velocity.Y = projectile.velocity.Y * 0.5f;
					}
					...
					else if ((flag) && ((double)projectile.velocity.Length() < (double)num3 * 0.6)) {
						vector5 = projectile.velocity;
						vector5.Normalize();
						vector5 *= num3 * 0.6f;
						projectile.velocity = (projectile.velocity * (num4 - 1f) + vector5) / num4;
					} else projectile.velocity *= 0.8f;
				...
				if (num6 < num3 + 10f || num6 == 0f) projectile.Kill();
				else {
					vector6.Normalize();
					vector6 *= num3;
		...
		public static void SpearAI(int index, float protractSpeed = 1.5f, float retractSpeed = 1.4f, ExtraAction action = null, ExtraAction initialize = null) {
			...
			if (!Main.player[projectile.owner].frozen) {
				...
				if (Main.player[projectile.owner].itemAnimation < Main.player[projectile.owner].itemAnimationMax / 3) projectile.ai[0] -= retractSpeed;
				else projectile.ai[0] += protractSpeed;
			}
		...
		public static void FlailAI(int index, float initialRange = 160f, float weaponOutRange = 300f, float retractRange = 100f, ExtraAction action = null, ExtraAction initialize = null) {
			...
			if (projectile.ai[0] == 0f) {
				...
				if (!Main.player[projectile.owner].channel || num3 > weaponOutRange || !projectile.tileCollide) {
					...
					projectile.tileCollide = false;
					if (num3 < 20f) projectile.Kill();
				}
				if (!projectile.tileCollide) num5 *= 2f;
				...
				if (num3 > (float)num6 || !projectile.tileCollide) {
					...
					if (Main.player[projectile.owner].velocity.X == 0f) projectile.velocity.X = projectile.velocity.X * 0.96f;
				}
			}

			if (projectile.velocity.X < 0f) projectile.rotation -= (Math.Abs(projectile.velocity.X) + Math.Abs(projectile.velocity.Y)) * 0.01f;
			else projectile.rotation += (Math.Abs(projectile.velocity.X) + Math.Abs(projectile.velocity.Y)) * 0.01f;
			if (action != null) action();
		}
		...
		public static bool FlailTileCollide(int index, Vector2 oldVelocity) {
			...
			if (oldVelocity.X != projectile.velocity.X) {
				if (Math.Abs(oldVelocity.X) > 4f) flag = true;
				projectile.velocity.X = -oldVelocity.X * 0.2f;
			}
			if (oldVelocity.Y != projectile.velocity.Y) {
				if (Math.Abs(oldVelocity.Y) > 4f) flag = true;
		...
		public static void BoomerangAI(int index, float retractTime = 30f, float speed = 9f, float speedAcceleration = 0.4f, ExtraAction action = null, ExtraAction initialize = null) {
			...
			if (projectile.ai[0] == 0f) {
				...
				if (projectile.velocity.X < num) {
					...
					if (projectile.velocity.X < 0f && num > 0f) projectile.velocity.X = projectile.velocity.X + speedAcceleration;
				...
				if (projectile.velocity.Y < num2) {
					...
					if (projectile.velocity.Y < 0f && num2 > 0f) projectile.velocity.Y = projectile.velocity.Y + speedAcceleration;
		...
		public static void ThrowingKnifeAI(int index, int airTime = 20, ExtraAction action = null, ExtraAction initialize = null) {
			...
			if (projectile.localAI[0] >= (float)airTime) {
				...
				projectile.velocity.X = projectile.velocity.X * 0.98f;
			} else projectile.rotation = (float)Math.Atan2((double)projectile.velocity.Y, (double)projectile.velocity.X) + 1.57f;
		...
		public static void Explode(int index, int sizeX, int sizeY, ExtraAction visualAction = null, bool weakerExplosion = false) {
			...
			projectile.position.X = projectile.position.X - (float)(projectile.width / 2);
			projectile.position.Y = projectile.position.Y - (float)(projectile.height / 2);
			if (weakerExplosion) projectile.damage = (int)(projectile.damage * .75f);
			projectile.Damage();
			...
			if (visualAction == null) {
				...
				for (int k = 0; k < 2; k++) {
					...
					if (k == 1) scaleFactor = 0.8f;
					int num3 = Gore.NewGore(null, new Vector2(projectile.position.X, projectile.position.Y), default(Vector2), Main.rand.Next(61, 64), 1f);
					...
					num3 = Gore.NewGore(null, new Vector2(projectile.position.X, projectile.position.Y), default(Vector2), Main.rand.Next(61, 64), 1f);
					Main.gore[num3].velocity *= scaleFactor;
		...
		public static void DrawString(int index, Vector2 to = default(Vector2)) {
			...
			if (to != default(Vector2)) vector = to;
			...
			if (!projectile.counterweight) {
				int num3 = -1;
				if (projectile.position.X + (float)(projectile.width / 2) < Main.player[projectile.owner].position.X + (float)(Main.player[projectile.owner].width / 2)) num3 = 1;
				num3 *= -1;
				Main.player[projectile.owner].itemRotation = (float)Math.Atan2((double)(num2 * (float)num3), (double)(num * (float)num3));
			}

			bool flag = true;
			if (num == 0f && num2 == 0f) flag = false;
			else {
			...
			while (flag) {
				...
				float num7 = num6;
				if (float.IsNaN(num6) || float.IsNaN(num7)) flag = false;
				else {
					if (num6 < 20f) {
						num5 = num6 - 8f;
						flag = false;
					...
					if (color.R < 75) color.R = 75;
					if (color.G < 75) color.G = 75;
					if (color.B < 75) color.B = 75;
					switch (stringColor) {
						case 13:
		...
		public static void DrawChain(int index, Vector2 to, string chainPath, bool electric = false, int damage = 0, bool zipline = false, float velocityX = 0, float velocityY = 0) {
			Texture2D texture = (Texture2D)ModContent.Request<Texture2D>(chainPath);
			Projectile projectile = Main.projectile[index];
			...
			bool flag = true;
			if ((float.IsNaN(vector.X) && float.IsNaN(vector.Y)) || (float.IsNaN(vector2.X) && float.IsNaN(vector2.Y))) flag = false;
			while (flag) {
				if ((double)vector2.Length() < (double)num + 1.0) flag = false;
				else {
					...
					if (electric) Projectile.NewProjectile(null, vector.X, vector.Y, 0, 0, ModLoader.GetMod("SpiritMod").Find<ModProjectile>("ElectricChain").Type, damage, 0, Main.myPlayer);
					if (zipline) Projectile.NewProjectile(null, vector.X, vector.Y, 0, 0, ModLoader.GetMod("SpiritMod").Find<ModProjectile>("ZiplinePiece").Type, 1, 0, Main.myPlayer, velocityX, velocityY);
				}
			}
		}
